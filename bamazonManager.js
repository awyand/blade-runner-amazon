...
function viewProducts() {
  ...
  connection.query("SELECT * FROM products", function(err, products) {
    ...
    var rows = [["Product ID", "Name", "Department", "Price (Chin-Yen)", "Stock"]];
    // loop through products array
    for (i = 0; i < products.length; i++) {
      // store current product as a variable
      var currentProduct = products[i];
      // push row of data (array) to rows array
      rows.push([currentProduct.id, currentProduct.product_name, currentProduct.department_name, currentProduct.price, currentProduct.stock_quantity]);
    }
    // log table
      ...
      connection.query(`SELECT * FROM products WHERE id = ${answers.item}`, function(err, product) {
        ...
        connection.query(`UPDATE products SET stock_quantity = ${newQuantity} WHERE id = ${answers.item}`, function(err, res) {
          ...
          if (err) {
            ...
            setTimeout(function() {
              console.log(`Success! You added ${answers.amount} units to the ${productMatch.product_name} stock.\n`);
              promptManager();
            }, 10);
          } else {
  ...
  setTimeout(function() {
    ...
    inquirer.prompt([
      ...
      },
      {
        ...
        choices: ["Weapons", "Vehicles", "Devices", "Books", "Replicants"]
      }
    ]).then(function(answers) {
      // insert into db
      connection.query("INSERT INTO products SET ?",
    {
      ...
      if (err) {
        ...
        // log success message
        setTimeout(function() {
          console.log(`Success! You added new item "${answers.name}" to the ${answers.dept} Department with initial stock ${answers.stock} and price ${answers.price}\n`);
        }, 10)
      } else {
        console.log("We're sorry, there appears to be some sort of problem. Please try again later.\n");
      }
      ...
      if (answers.confirmOne && answers.confirmTwo) {
        ...
        connection.query(`DELETE FROM products WHERE id = ${answers.item}`, function(err, res) {
          if (err) {
            ...
            // log success message
            setTimeout(function() {
              console.log(`Successfully deleted ${productName} from inventory.`);
            }, 10);
          } else {
            console.log("We're sorry, there appears to be some sort of problem. Please try again later.\n");
          }
