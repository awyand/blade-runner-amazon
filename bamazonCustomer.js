...
function displayItemsForSale() {
  ...
  if (isNewSession) {
    ...
    clear();
    // welcome message
    console.log("WELCOME TO BAMAZON: THE BLADE RUNNER AMAZON");
  ...
  connection.query("SELECT * FROM products", function(err, products) {
    ...
    var rows = [["Product ID", "Name", "Price (Chin-Yen)"]];
    ...
    for (i = 0; i < products.length; i++) {
      ...
      rows.push([currentProduct.id, currentProduct.product_name, currentProduct.price]);
        ...
        connection.query(`UPDATE products SET stock_quantity = ${itemMatch.stock_quantity - parseInt(answers.amount)}, product_sales = ${itemMatch.product_sales + totalCost} WHERE id = ${answers.item}`, function(err, res) {
          ...
          if (res.affectedRows === 1) {
            ...
            if (parseInt(answers.amount) === 1) {
              console.log(`*** Success! You purchased 1 ${itemMatch.product_name} for ${itemMatch.price} Chin Yen. Enjoy! ***`);
            } else {
              console.log(`*** Success! You purchased ${parseInt(answers.amount)} ${itemMatch.product_name}s for a total cost of ${totalCost} Chin Yen. Enjoy! ***`);
            }
            // call displayItemsForSale
            displayItemsForSale();
          } else {
